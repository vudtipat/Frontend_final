...
export default class Job_Annoucement_Create extends React.Component {
    ...
    onSave = async() => {
        ...
        var email = await AsyncStorage.getItem('email')
        fetch(url+'/Employer_Profile?want='+email, {
          method: 'GET',
        }).then((response) => response.json()).then((respone) => {
          if(respone.response == 'Pass')
          {
            ...
            fetch(url+'/Employer_Annoucment', {
              ...
              Alert.alert("กรุณาตรวจสอบรายละเอียดใหม่อีกครั้ง")
            }
...
{/*}
                                      <DropDownPicker
                                            ...
                                            containerStyle={{height: 50, width:'95%', marginTop:10}}
                                            searchable={true}
                                            placeholder="Select a job type.."
                                            style={{backgroundColor: '#EBEBEB'}}
                                            itemStyle={{  justifyContent: 'flex-start'  }}
                                            dropDownStyle={{backgroundColor: '#fafafa'}}
                                            onChangeItem={item => this.setState({jobType: item.value})}
...
{*/}
                                ...
                                <View style={{flex:1, marginTop:10, width:'100%'}}>
                                    ...
                                    <Text style={{fontSize:20, color:"#450887", marginTop:0, marginLeft:10, alignSelf:'flex-start'}}>Description</Text>

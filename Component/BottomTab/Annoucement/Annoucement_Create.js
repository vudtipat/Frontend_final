...
import { Alert, AsyncStorage, TouchableOpacity, View ,Text,TextInput, TouchableWithoutFeedback,Keyboard, ScrollView} from 'react-native';
import {url} from '../../var';
...
export default class Annoucement_Create extends React.Component {
    ...
    createAnnoucement = async() => {
        ...
        var email = await AsyncStorage.getItem('email')
        fetch(url+'/Employee_Profile?want='+email, {
          method: 'GET',
        }).then((response) => response.json()).then((respone) => {
          if(respone.response == 'Pass')
          {
              ...
              fetch(url+'/Employee_Annoucment', {
                ...
                Alert.alert("กรุณาตรวจสอบข้อมูลใหม่อีกครั้ง")
              }
